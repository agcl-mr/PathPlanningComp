...
void polygon2D::reconstruct_edges(void) {
	...
	for (int i = 0; i < edges.size(); i++) {
		std::cout << edges.at(i).x1 * 50 + 50.5 << ", ";
	}
	std::cout << "];\n";
	std::cout << "y_poly = [";
	for (int i = 0; i < edges.size(); i++) {
		std::cout << -edges.at(i).y1 * 50 + 50.5 << ", ";
	}
	std::cout << "];\n";
}
...
void voronoi_algo::merge_strips(int row, int index2, polygon2D* obstacle, std::vector<std::vector<coord>>* strips_list, std::string id) {
	// start/continue a vertice list by adding a confirmed point. left end of the strip given by row = i, and index = index2
	obstacle->vertices.push_back(strips_list->at(row).at(index2).a);
	// update left and bottom bounds using row and strip-left-end information
	...
	// checks for array out of bound limit - bottom
	if (row + 1 < GRID_HEIGHT) {
		// ensures that there is a strip element in next row. Otherwise, continue;
		if (!strips_list->at(row + 1).empty()) {
			// for each strip in next row
			for (int i = 0; i < strips_list->at(row + 1).size(); i++) {
				if ((strips_list->at(row + 1).at(i).a % GRID_WIDTH <= strips_list->at(row).at(index2).b % GRID_WIDTH)
					...
					merge_strips(row + 1, i, obstacle, strips_list, id);
					break;
					// bottom and right bounds will be taken care by last merge_strip recursion call. So safe to break out of loop
				}
			}
		}
	}

	// adding the right counter parts of strips, at required place
	...
	// this strip is consumed up so delete it from strips list.
	strips_list->at(row).erase(strips_list->at(row).begin() + index2);
}

/*void voronoi_algo::merge_strips_up(int row, int index2, polygon2D* obstacle, std::vector<std::vector<coord>>* strips_list, std::string id) {
	// start/continue a vertice list (going up) by adding a confirmed point. right end of the strip given by row = i, and index = index2
	obstacle->vertices.push_back(strips_list->at(row).at(index2).b);
	// update right and top bounds using row and strip-right-end information
	if (obstacle->looseBounds.top > row)
		obstacle->looseBounds.top = row;
	if (obstacle->looseBounds.right < strips_list->at(row).at(index2).b % GRID_WIDTH)
		obstacle->looseBounds.right = strips_list->at(row).at(index2).b % GRID_WIDTH;

	// checks for array out of bound limit - top
	if (row - 1 >= 0) {
		// ensures that there is a strip element in upper row. Otherwise, continue;
		if (!strips_list->at(row - 1).empty()) {
			// for each strip in next row
			for (int i = strips_list->at(row - 1).size() - 1; i >= 0; i--) {
				if ((strips_list->at(row - 1).at(i).a % GRID_WIDTH >= strips_list->at(row).at(index2).b % GRID_WIDTH)
					|| (strips_list->at(row - 1).at(i).b % GRID_WIDTH <= strips_list->at(row).at(index2).a % GRID_WIDTH)) {

				}
				else {
					std::string new_id = id + "_" + std::to_string(row) + "_" + std::to_string(index2);
					merge_strips_up(row - 1, i, obstacle, strips_list, new_id);
					std::cout << "[UP] :  i = " << i << ", size = " << strips_list->at(row - 1).size() << "\n";
					//i++;
				}
			}
		}
	...
	// adding the left counter parts of strips, at required place
	obstacle->vertices.push_back(strips_list->at(row).at(index2).a);
	if (obstacle->looseBounds.bottom < row)
		obstacle->looseBounds.bottom = row;
	if (obstacle->looseBounds.left > strips_list->at(row).at(index2).a % GRID_WIDTH)
		...
		if (!strips_list->at(row + 1).empty()) {
			...
			for (int i = 0; i < strips_list->at(row + 1).size(); i++) {
				if ((strips_list->at(row + 1).at(i).a % GRID_WIDTH >= strips_list->at(row).at(index2).b % GRID_WIDTH)
					...
					std::string new_id = id + "_" + std::to_string(row) + "_" + std::to_string(index2);
					merge_strips(row + 1, i, obstacle, strips_list, new_id);
					i--;
					if (row - 1 >= 0) {
						if (!strips_list->at(row - 1).empty()) {
							for (int j = strips_list->at(row - 1).size() - 1; j >= 0; j--) {
								if ((strips_list->at(row - 1).at(j).a % GRID_WIDTH >= strips_list->at(row).at(index2).b % GRID_WIDTH)
									|| (strips_list->at(row - 1).at(j).b % GRID_WIDTH <= strips_list->at(row).at(index2).a % GRID_WIDTH)) {

								}
								else {
									merge_strips_up(row - 1, j, obstacle, strips_list, new_id + "up");
									j++;
								}
							}
						}
...
void voronoi_algo::locate_obstacles(void) {
	...
	for (int i = 0; i < strips.size(); i++) {
		...
		while (!strips[i].empty()) {
			...
			merge_strips(i, 0, &obstacle, &strips, "R"+i);
...
void voronoi_algo::locate_obstacles(void) {
	...
	for (int i = 0; i < strips.size(); i++) {
		...
		while (!strips[i].empty()) {
			obstacle.looseBounds = rectangularBound(strips.at(i).at(0).a % GRID_WIDTH, strips.at(i).at(0).b % GRID_WIDTH, i, i);
			merge_strips(i, 0, &obstacle, &strips, "none");
			if (obstacle.vertices.size() < 3)
				continue;
			obstacles.push_back(obstacle);
			obstacles.at(obstacles.size() - 1).uniqueID = obstacles.size();
